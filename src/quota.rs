...
pub fn wrap_streaming_body_with_codex_rates(
    ...
    Body::from_stream(SseScanner::new(body.into_data_stream(), on_complete, Some(on_rate)))
}
                    ...
                    if v.get("type").and_then(|v| v.as_str()) == Some("codex.rate_limits") {
                        ...
                        if let Some(callback) = &self.on_codex_rate {
                            callback(CodexRateUpdate {
                                primary_used_percent: primary.and_then(|w| w.get("used_percent")).and_then(|v| v.as_f64()),
                                primary_reset_at: primary.and_then(|w| w.get("reset_at")).and_then(|v| v.as_u64()),
                                secondary_used_percent: secondary.and_then(|w| w.get("used_percent")).and_then(|v| v.as_f64()),
                                secondary_reset_at: secondary.and_then(|w| w.get("reset_at")).and_then(|v| v.as_u64()),
                            });
                        }
                    }
                    // OpenAI Responses SSE: response.completed carries the final
                    // usage object under `response.usage`.
                    let response_usage = v.get("response").and_then(|r| r.get("usage"))
                        .or_else(|| {
                            if v.get("type").and_then(|t| t.as_str())
                                .is_some_and(|t| t == "response.completed" || t == "response.incomplete")
                            { v.get("usage") } else { None }
                        });
