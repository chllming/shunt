...
async fn handle_tunnel(socket: WebSocket, state: RelayState) {
        ...
        Some(Ok(Message::Text(text))) => match serde_json::from_str::<ClientFrame>(&text) {
            Ok(ClientFrame::Register { subdomain, token }) => {
                if token != *state.allowed_token {
                    let _ = sink
                        .send(Message::Text(
                            serde_json::to_string(&RelayFrame::Deny {
                ...
                let _ = sink
                    .send(Message::Text(
                        serde_json::to_string(&RelayFrame::Ack {
                            subdomain: &subdomain,
                        })
                        .unwrap(),
                    ))
    ...
    state
        .tunnels
        .write()
        .insert(subdomain.clone(), TunnelHandle { tx: tunnel_tx });
    ...
    tokio::spawn(async move {
        while let Some(req) = tunnel_rx.recv().await {
            ...
            if ws_tx2.send(Message::Text(text)).await.is_err() {
                break;
            }
        }
    });
...
async fn proxy_handler(State(state): State<RelayState>, req: Request<Body>) -> Response {
