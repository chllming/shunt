...
impl Forwarder {
    ...
    pub async fn forward(
        ...
        account
            .provider
            .inject_auth_headers(&mut upstream_headers, token)
    ...
    pub async fn forward_codex(
        ...
        for (name, value) in client_headers {
            ...
            if !allowed || sensitive || is_hop_by_hop(&lower) {
                continue;
            }
            if let (Ok(n), Ok(v)) = (
            ...
            if let Some(account_id) = oauth.chatgpt_account_id.as_deref() {
                upstream_headers.insert(
                    ...
                    reqwest::header::HeaderValue::from_str(account_id)
                        .context("invalid ChatGPT account id")?,
                );
            }
        ...
        let upstream_resp = self
            ...
            .send()
            .await
            .context("Codex upstream request failed")?;
