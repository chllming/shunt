...
pub fn write_codex_auth_file(cred: &OAuthCredential) {
    let Some(ref id_token) = cred.id_token else {
        return;
    };
...
pub fn read_codex_credentials() -> Option<OAuthCredential> {
    ...
    let expires_at = jwt_exp_ms(&raw.tokens.access_token).unwrap_or(now_ms + 3600 * 1000); // default: 1 hour from now

    let claims = raw
        .tokens
        .id_token
        .as_deref()
        .and_then(parse_chatgpt_claims);
...
pub async fn refresh_openai_token(cred: &OAuthCredential) -> Result<OAuthCredential> {
    ...
    let body: serde_json::Value = resp
        .json()
        .await
        .context("OpenAI token refresh: invalid JSON")?;
...
pub async fn run_openai_oauth_flow() -> Result<OAuthCredential> {
    ...
    let device_auth_id = info["device_auth_id"]
        .as_str()
        .context("missing device_auth_id")?
        .to_owned();
    let user_code = info["user_code"]
        .as_str()
...
mod tests {
    ...
    fn test_rand_bytes_not_all_zeros() {
        ...
        assert!(
            ...
            "rand_bytes must not return all-zero output"
        );
    }
    ...
    fn test_jwt_exp_ms_sanity_cap() {
        ...
        assert!(
            jwt_exp_ms(&make_token(-3600)).is_none(),
            "expired token must return None"
        );
        // Valid 1h: should return Some
        assert!(
            jwt_exp_ms(&make_token(3600)).is_some(),
            "1h-future token must return Some"
        );
        // Valid 24h: should return Some
        assert!(
            jwt_exp_ms(&make_token(86400)).is_some(),
            "24h-future token must return Some"
        );
        // 26h in the future: exceeds 25h cap → None
        assert!(
            jwt_exp_ms(&make_token(26 * 3600)).is_none(),
            "26h-future token must return None (forged exp)"
        );
    }
