...
fn canonical_tools(v: &serde_json::Value) -> String {
    match v.as_array() {
        None => "null".into(),
        Some(arr) => {
            let mut names: Vec<_> = arr.iter().filter_map(|t| t["name"].as_str()).collect();
            names.sort_unstable();
            names.join(",")
        }
    }
...
fn most_urgent_window(d: &AccountRoutingData) -> (f64, Option<u64>) {
    ...
    let binding_reset = if d.util_5h >= d.util_7d {
        d.reset_5h_secs
    } else {
        d.reset_7d_secs
    };
...
pub fn pick_account<'a>(
    ...
    let chosen = match strategy {
        ...
        RoutingStrategy::Cushion => candidates.iter().copied().min_by(|a, b| {
            ...
            a_binding
                .partial_cmp(&b_binding)
                .unwrap_or(Ordering::Equal)
                .then_with(|| {
                    a_secondary
                        .partial_cmp(&b_secondary)
                        .unwrap_or(Ordering::Equal)
                })
        })?,

        // ── Maximus: time-weighted dual-window scorer ────────────────────────
        RoutingStrategy::Maximus => candidates.iter().copied().max_by(|a, b| {
            let da = get_data(&a.name, snap);
            let db = get_data(&b.name, snap);
        ...
        RoutingStrategy::Reaper => candidates.iter().copied().min_by(|a, b| {
            let da = get_data(&a.name, snap);
            let db = get_data(&b.name, snap);
            ...
            let a_expiring = ra
                .map(|r| r.saturating_sub(snap.now_secs) <= expiry_soon_secs)
                .unwrap_or(false);
            let b_expiring = rb
                .map(|r| r.saturating_sub(snap.now_secs) <= expiry_soon_secs)
                .unwrap_or(false);
...
mod tests {
    ...
    fn fingerprint_stable_for_same_content() {
        let body =
            br#"{"system":"You are helpful","messages":[{"role":"user","content":"hello"}]}"#;
    ...
    fn reaper_prefers_expiring_soon() {
        ...
        assert_eq!(
            pick(&accounts, &state, RoutingStrategy::Reaper).as_deref(),
            Some("expiring"),
    ...
    fn all_strategies_skip_exhausted() {
        for strategy in [
            ...
            assert_eq!(
                pick(&accounts, &state, strategy).as_deref(),
                Some("fresh"),
    ...
    fn cushion_picks_lowest_binding() {
        ...
        assert_eq!(
            pick(&accounts, &state, RoutingStrategy::Cushion).as_deref(),
            Some("light"),
    ...
    fn maximus_prefers_imminent_reset_over_raw_utilization() {
        ...
        assert_eq!(
            pick(&accounts, &state, RoutingStrategy::Maximus).as_deref(),
            Some("almost_reset"),
    ...
    fn health_check_failed_cleared_allows_routing() {
        ...
        assert!(
            pick(&accounts, &state, RoutingStrategy::Maximus).is_none(),
