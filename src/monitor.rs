...
fn style_dkgreen() -> Style {
    Style::default().fg(DK_GREEN)
}
fn style_dim() -> Style {
...
const MODEL_PRESETS: &[(&str, &str, &str)] = &[
    ...
    (
        "Opus 4.8",
        "Flagship Opus · complex agentic tasks",
        "claude-opus-4-8",
    ),
...
impl ModelPicker {
    ...
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % MODEL_PRESETS.len();
    }
    fn selected_id(&self) -> &str {
        MODEL_PRESETS[self.cursor].2
    }
}
...
const STRATEGY_PRESETS: &[(&str, &str, &str)] = &[
    ...
    (
        "Reaper",
        "Use-it-or-lose-it · drain expiring first",
        "reaper",
    ),
...
impl StrategyPicker {
    ...
    fn up(&mut self) {
        self.cursor = if self.cursor == 0 {
            STRATEGY_PRESETS.len() - 1
        } else {
            self.cursor - 1
        };
    }
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % STRATEGY_PRESETS.len();
    }
    fn selected_id(&self) -> &str {
        STRATEGY_PRESETS[self.cursor].2
    }
}
...
impl Menu {
    ...
    fn up(&mut self) {
        self.cursor = self.cursor.checked_sub(1).unwrap_or(MENU_ITEMS - 1);
    }
    fn down(&mut self) {
...
impl SpeedPicker {
    ...
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % SPEED_PRESETS.len();
    }
    fn selected(&self) -> u64 {
        SPEED_PRESETS[self.cursor]
    }
}
...
impl BurstLimitPicker {
    fn new(current: u32) -> Self {
        let cursor = BURST_LIMIT_PRESETS
            .iter()
            .position(|&v| v == current)
            .unwrap_or(3); // default to 10
        Self { cursor }
    }
    fn up(&mut self) {
        self.cursor = self
            .cursor
            .checked_sub(1)
            .unwrap_or(BURST_LIMIT_PRESETS.len() - 1);
    }
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % BURST_LIMIT_PRESETS.len();
    }
    fn selected(&self) -> u32 {
        BURST_LIMIT_PRESETS[self.cursor]
    }
}
...
impl FallbackPicker {
    ...
    fn up(&mut self) {
        self.cursor = self
            .cursor
            .checked_sub(1)
            .unwrap_or(FALLBACK_PRESETS.len() - 1);
    }
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % FALLBACK_PRESETS.len();
    }
    fn selected_id(&self) -> &str {
        FALLBACK_PRESETS[self.cursor].0
    }
}
...
impl EffortPicker {
    ...
    fn up(&mut self) {
        self.cursor = self
            .cursor
            .checked_sub(1)
            .unwrap_or(EFFORT_PRESETS.len() - 1);
    }
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % EFFORT_PRESETS.len();
    }
    fn selected_id(&self) -> &str {
        EFFORT_PRESETS[self.cursor].0
    }
}
...
impl ThinkingPicker {
    ...
    fn up(&mut self) {
        self.cursor = self
            .cursor
            .checked_sub(1)
            .unwrap_or(THINKING_PRESETS.len() - 1);
    }
    fn down(&mut self) {
        self.cursor = (self.cursor + 1) % THINKING_PRESETS.len();
    }
    fn selected_id(&self) -> &str {
        THINKING_PRESETS[self.cursor].0
    }
}
                        ...
                        KeyCode::Enter => {
                            match m.cursor {
                                ...
                                1 => {
                                    // set model
                                    model_picker =
                                        Some(ModelPicker::new(model_override.as_deref()));
                                }
                                2 => {
                                    // strategy
                                    strategy_picker =
                                        Some(StrategyPicker::new(current_strategy.as_deref()));
                                }
                                3 => {
                                    // toggle mute
                                ...
                                5 => {
                                    // burst limit
                                    burst_limit_picker =
                                        Some(BurstLimitPicker::new(current_burst_limit));
                                }
                                6 => {
                                    // fallback model
                                    fallback_picker =
                                        Some(FallbackPicker::new(current_fallback.as_deref()));
                                }
                                7 => {
                                    // effort
                                    effort_picker =
                                        Some(EffortPicker::new(current_effort.as_deref()));
                                }
                                8 => {
                                    // thinking
                                    thinking_picker =
                                        Some(ThinkingPicker::new(current_thinking.as_deref()));
                                }
                                _ => {}
                            }
                        }
...
fn draw(
    ...
    let any_overlay = menu.is_some()
        || picker.is_some()
        || model_picker.is_some()
        || strategy_picker.is_some()
        || speed_picker.is_some()
        || burst_limit_picker.is_some()
        || fallback_picker.is_some()
        || effort_picker.is_some()
        || thinking_picker.is_some();
    ...
    if let Some(p) = picker {
        draw_picker(f, p, current_strategy.as_deref(), area);
    }
    if let Some(mp) = model_picker {
        draw_model_picker(f, mp, model_override.as_deref(), area);
    }
    if let Some(sp) = strategy_picker {
        draw_strategy_picker(f, sp, current_strategy.as_deref(), area);
    }
    if let Some(sp) = speed_picker {
        draw_speed_picker(f, sp, refresh_ms, area);
    }
    if let Some(bp) = burst_limit_picker {
        draw_burst_limit_picker(f, bp, current_burst_limit, area);
    }
    if let Some(fp) = fallback_picker {
        draw_fallback_picker(f, fp, current_fallback, area);
    }
    if let Some(ep) = effort_picker {
        draw_effort_picker(f, ep, current_effort, area);
    }
    if let Some(tp) = thinking_picker {
        draw_thinking_picker(f, tp, current_thinking, area);
    }
    if show_help {
...
fn draw_header(
    ...
    let uptime_span = state.as_ref().and_then(|s| s.started_ms).map(|ms| {
        let now_ms = now_ms();
    ...
    if let Some(sv) = state
        .as_ref()
        .and_then(|s| s.savings.as_ref())
        .filter(|sv| sv.today_cost_usd > 0.001)
    {
        spans.push(Span::styled("  ·  ", style_dim()));
    ...
    if let Some(s) = state.as_ref() {
        spans.push(Span::styled("  ·  ", style_dim()));
        let style = if s.effective_free == 0 {
            style_red().add_modifier(Modifier::BOLD)
        } else if s.effective_free <= 2 {
            style_yellow()
        } else {
            style_green()
        };
    ...
    if let Some(s) = state.as_ref() {
        let anthropic: Vec<&AccountStatus> = s
            .accounts
            .iter()
        ...
        if !anthropic.is_empty() && anthropic.iter().all(|a| !a.available) {
            ...
            if let Some(d) = soonest {
                spans.push(Span::styled(
                    format!(" — next in {}", fmt_duration_ms(d)),
                    style_red(),
                ));
            }
        }
...
fn draw_footer(f: &mut Frame, area: Rect, overlay_open: bool, focus: Focus) {
    let hint = if overlay_open {
        ...
        Line::from(vec![
            ...
            sep(),
            Span::styled("tab", style_green()),
            Span::styled(" focus", style_dim()),
            sep(),
            Span::styled("↑↓", style_green()),
            scroll_hint,
            sep(),
            ...
            sep(),
            Span::styled("s", style_green()),
            Span::styled(" settings", style_dim()),
            sep(),
            Span::styled("?", style_green()),
    ...
    let lines: Vec<Line> = match error {
        Some(FetchError::NotRunning) if remote => vec![
            ...
            Line::from(vec![Span::styled(
                "  Is the host still running shunt?",
                style_dim(),
            )]),
            Line::from(vec![
...
fn panel_title_style(focused: bool) -> Style {
    if focused {
        style_green().add_modifier(Modifier::BOLD)
    } else {
        style_dim()
    }
}
    ...
    for acc in &s.accounts {
        ...
        if acc.provider == "anthropic" || acc.provider.is_empty() {
            if acc.utilization_5h > 0.0 || acc.reset_5h.is_some() {
                lines.push(util_bar_line(
                    "5h",
                    acc.utilization_5h,
                    acc.reset_5h,
                    acc.status_5h.as_deref(),
                ));
            }
...
fn util_bar_line(
    ...
    let bar_color = if exhausted || util >= 0.8 {
        RED
    } else if util >= 0.5 {
        YELLOW
    } else {
    ...
    let reset_str = reset
        .map(|reset_secs| {
            ...
            if reset_secs > now_secs {
                format!(
                    "  resets {}",
                    fmt_duration_ms((reset_secs - now_secs) * 1000)
                )
            } else {
                String::new()
            }
...
fn draw_request_log(f: &mut Frame, area: Rect, s: &StatusResponse, scroll: usize, focused: bool) {
    ...
    let clf_per_min = s
        ...
        .count();
    let clf_str = if clf_per_min > 0 {
        format!("  {clf_per_min} clf/min")
    } else {
        String::new()
    };
    ...
    if s.recent_requests.is_empty() {
        f.render_widget(
            Paragraph::new(Line::from(Span::styled("  no requests yet", style_dim()))),
            inner,
        );
        return;
    }
...
fn draw_picker(f: &mut Frame, picker: &Picker, strategy: Option<&str>, area: Rect) {
    ...
    f.render_widget(
        Table::new(rows, [Constraint::Min(0)]).column_spacing(0),
        inner,
    );
}
...
fn draw_model_picker(f: &mut Frame, mp: &ModelPicker, current: Option<&str>, area: Rect) {
    ...
    let rows: Vec<Row> = MODEL_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &(name, desc, id))| {
...
fn draw_strategy_picker(f: &mut Frame, sp: &StrategyPicker, current: Option<&str>, area: Rect) {
    ...
    let rows: Vec<Row> = STRATEGY_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &(name, desc, id))| {
...
fn draw_menu(
    ...
    let items: &[(&str, String)] = &[
        ...
        (
            "burst limit",
            if current_burst_limit == 0 {
                "off".to_owned()
            } else {
                format!("{}/min", current_burst_limit)
            },
        ),
        (
            "fallback",
            match current_fallback.as_deref() {
                None => "auto".to_owned(),
        ...
        (
            "effort",
            current_effort.as_deref().unwrap_or("auto").to_owned(),
        ),
        (
            "thinking",
            match current_thinking.as_deref() {
                None => "auto".to_owned(),
    ...
    f.render_widget(
        Table::new(rows, [Constraint::Min(20), Constraint::Length(14)]).column_spacing(2),
        inner,
    );
}
...
fn draw_speed_picker(f: &mut Frame, sp: &SpeedPicker, current_ms: u64, area: Rect) {
    ...
    let rows: Vec<Row> = SPEED_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &ms)| {
            let is_sel = i == sp.cursor;
...
fn draw_burst_limit_picker(f: &mut Frame, bp: &BurstLimitPicker, current: u32, area: Rect) {
    ...
    let rows: Vec<Row> = BURST_LIMIT_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &val)| {
...
fn draw_fallback_picker(f: &mut Frame, fp: &FallbackPicker, current: &Option<String>, area: Rect) {
    ...
    let rows: Vec<Row> = FALLBACK_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &(id, desc))| {
    ...
    f.render_widget(
        Table::new(rows, [Constraint::Length(4), Constraint::Min(0)]).column_spacing(1),
        inner,
    );
}
...
fn draw_effort_picker(f: &mut Frame, ep: &EffortPicker, current: &Option<String>, area: Rect) {
    ...
    let rows: Vec<Row> = EFFORT_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &(id, desc))| {
...
fn draw_thinking_picker(f: &mut Frame, tp: &ThinkingPicker, current: &Option<String>, area: Rect) {
    ...
    let rows: Vec<Row> = THINKING_PRESETS
        .iter()
        .enumerate()
        .map(|(i, &(id, desc))| {
