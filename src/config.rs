...
pub fn load_config(path: Option<&Path>) -> Result<Config> {
    ...
    let server = ServerConfig {
        ...
        port: if schema_version >= CONFIG_SCHEMA_VERSION {
            pools.claude.port
        } else {
            raw.server.port
        },
    ...
    for (kind, a) in &account_specs {
        ...
        let allow_cli_import = schema_version < CONFIG_SCHEMA_VERSION
            || matches!(
                a.credential_source.as_deref(),
                Some("codex_auth_file" | "claude_credentials_file" | "local_cli")
            );
...
pub fn migrate_config_file(path: &Path, apply: bool, env_file: Option<&Path>) -> Result<String> {
    ...
    if root.get("schema_version").and_then(toml::Value::as_integer)
