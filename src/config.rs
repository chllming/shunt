...
impl CredentialsStore {
    ...
    pub fn get(&self, pool: PoolKind, name: &str) -> Option<Credential> {
        if pool == PoolKind::Legacy {
            ...
            self.pools.get(pool.as_str()).and_then(|m| m.get(name)).cloned()
        }
    }
    ...
    pub fn insert(&mut self, pool: PoolKind, name: String, credential: Credential) {
        if pool == PoolKind::Legacy {
            ...
            self.pools.entry(pool.as_str().to_owned()).or_default().insert(name, credential);
        }
    }
...
impl Default for PoolsConfig {
    fn default() -> Self {
        Self { claude: PoolConfig::claude_default(), codex: PoolConfig::codex_default() }
    }
}
...
impl Default for ClassifierConfig {
    fn default() -> Self {
        Self { enabled: true, upstream_url: None, model: None, fail_closed: true }
    }
}
...
impl NetworkPolicy {
    ...
    pub fn permits(self, requested: Self) -> bool {
        let rank = |v| match v { Self::None => 0, Self::Allowlisted => 1, Self::Unrestricted => 2 };
...
fn load_env_file(path: &Path) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if metadata.permissions().mode() & 0o077 != 0 {
            bail!("secrets.env_file must not be accessible by group/others (chmod 600): {}", path.display());
        }
    }
    ...
    for (line_no, raw_line) in text.lines().enumerate() {
        ...
        let Some((key, raw_value)) = line.split_once('=') else {
            bail!("Invalid env assignment at {}:{}", path.display(), line_no + 1);
        };
        let key = key.trim();
        if key.is_empty() || !key.bytes().enumerate().all(|(i, b)| {
            b == b'_' || b.is_ascii_alphabetic() || (i > 0 && b.is_ascii_digit())
        }) {
            bail!("Invalid environment variable name at {}:{}", path.display(), line_no + 1);
        }
...
fn resolve_pool(raw: Option<RawPool>, kind: PoolKind) -> PoolConfig {
    ...
    let key_env = match kind { PoolKind::Claude => "ANTHROPIC_API_KEY", _ => "OPENAI_API_KEY" };
    PoolConfig {
        port: raw.port.unwrap_or(defaults.port),
        routing_strategy: raw.routing_strategy.as_deref()
...
pub fn load_config(path: Option<&Path>) -> Result<Config> {
    ...
    let primary_provider_derived = account_specs.first()
        .map(|(kind, a)| a.provider.as_deref().map(Provider::from_str).unwrap_or_else(|| match kind {
    ...
    let classifier_account = raw.classifier.account.clone().or(raw.server.classifier_account.clone())
        .or_else(|| std::env::var("SHUNT_CLASSIFIER_ACCOUNT").ok())
        .map(|n| scoped_account_name(PoolKind::Claude, &n, schema_version));
    let classifier_fallback_account = raw.classifier.fallback_account.clone()
        .or(raw.server.classifier_fallback_account.clone())
        .or_else(|| std::env::var("SHUNT_CLASSIFIER_FALLBACK_ACCOUNT").ok())
        .map(|n| scoped_account_name(PoolKind::Claude, &n, schema_version));
    let classifier_system_prompt_path = raw.classifier.system_prompt_path.as_ref()
    ...
    if schema_version < CONFIG_SCHEMA_VERSION {
        ...
        api_overflow.daily_budget_usd = std::env::var("SHUNT_API_OVERFLOW_DAILY_BUDGET_USD")
            .ok().and_then(|v| v.parse().ok()).unwrap_or(api_overflow.daily_budget_usd);
    ...
    for (kind, a) in &account_specs {
        let provider = a.provider.as_deref().map(Provider::from_str).unwrap_or_else(|| match kind {
            PoolKind::Codex => Provider::OpenAI,
            _ => Provider::Anthropic,
        });
        match kind {
            PoolKind::Claude if !matches!(provider, Provider::Anthropic | Provider::AnthropicApi) => {
                bail!("Claude pool account '{}' must use provider anthropic or anthropic-api", a.name);
            }
    ...
    for (kind, pool, provider) in [
        ...
        if !pool.overflow.enabled { continue; }
        let configured = pool.overflow.account.clone().unwrap_or_else(|| "api-overflow".to_owned());
    ...
    if let Some(url) = classifier.upstream_url.as_deref() {
        validate_upstream_url(url, true).with_context(|| "classifier.upstream_url failed validation")?;
    }
    let bridge = BridgeConfig {
        ...
        network_ceiling: raw.bridge.network_ceiling.as_deref()
            .and_then(NetworkPolicy::parse).unwrap_or(NetworkPolicy::Allowlisted),
    ...
    Ok(Config {
        ...
        api_overflow,
        pools,
        secrets: SecretsConfig { env_file: raw.secrets.env_file },
        classifier,
        bridge,
    })
}
...
pub fn migrate_config_file(path: &Path, apply: bool, env_file: Option<&Path>) -> Result<String> {
    ...
    let classifier_account = root.get("server").and_then(toml::Value::as_table)
        .and_then(|t| t.get("classifier_account")).and_then(toml::Value::as_str).map(ToOwned::to_owned);
    let local_classifier = classifier_account.as_deref().and_then(|name| {
        accounts.iter().find_map(|account| {
            let table = account.as_table()?;
            if table.get("name").and_then(toml::Value::as_str) != Some(name) { return None; }
            if table.get("provider").and_then(toml::Value::as_str) != Some("local") { return None; }
            Some((
    ...
    let old_port = root.get("server").and_then(toml::Value::as_table)
        .and_then(|t| t.get("port")).and_then(toml::Value::as_integer).unwrap_or(8082);
    ...
    for mut account in accounts {
        let provider = account.as_table().and_then(|t| t.get("provider"))
            .and_then(toml::Value::as_str).unwrap_or("anthropic").to_ascii_lowercase();
        let account_name = account.as_table().and_then(|t| t.get("name"))
        ...
        if let Some(table) = account.as_table_mut() {
            ...
            if let Some(source) = source {
                table.entry("credential_source").or_insert(toml::Value::String(source.into()));
            }
        }
    ...
    claude.insert("routing_strategy".into(), toml::Value::String("maximus".into()));
    claude.insert("accounts".into(), toml::Value::Array(claude_accounts.clone()));
    if let Some(overflow) = legacy_overflow { claude.insert("overflow".into(), overflow); }
    let mut codex = toml::Table::new();
    codex.insert("port".into(), toml::Value::Integer(8083));
    codex.insert("routing_strategy".into(), toml::Value::String("maximus".into()));
    codex.insert("accounts".into(), toml::Value::Array(codex_accounts.clone()));
    ...
    if !legacy_accounts.is_empty() { root.insert("accounts".into(), toml::Value::Array(legacy_accounts)); }
    ...
    root.entry("bridge").or_insert_with(|| toml::Value::Table(toml::Table::from_iter([
        ("enabled".into(), toml::Value::Boolean(true)),
    ...
    let mut move_credential = |pool: PoolKind, account: &toml::Value| {
        let Some(name) = account.as_table().and_then(|t| t.get("name")).and_then(toml::Value::as_str) else { return };
        if let Some(credential) = store.accounts.remove(name) { store.insert(pool, name.to_owned(), credential); }
    };
    for account in &claude_accounts { move_credential(PoolKind::Claude, account); }
    for account in &codex_accounts { move_credential(PoolKind::Codex, account); }
    store.save()?;

    let state_backup = state_path().with_extension("json.bak-v1");
    if state_path().exists() {
        if !state_backup.exists() { std::fs::copy(state_path(), &state_backup)?; }
        let claude_names = claude_accounts.iter().filter_map(|a| a.as_table()?.get("name")?.as_str().map(ToOwned::to_owned)).collect();
        let codex_names = codex_accounts.iter().filter_map(|a| a.as_table()?.get("name")?.as_str().map(ToOwned::to_owned)).collect();
        migrate_state_account_names(&state_path(), &claude_names, &codex_names)?;
    }
    Ok(migrated)
}

fn migrate_state_account_names(path: &Path, claude_names: &std::collections::HashSet<String>, codex_names: &std::collections::HashSet<String>) -> Result<()> {
    let mut value: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(path)?)?;
    let scoped = |name: &str| {
        if claude_names.contains(name) { format!("claude/{name}") }
        else if codex_names.contains(name) { format!("codex/{name}") }
        else { name.to_owned() }
    };
    for field in ["accounts", "quota", "rate_limits", "per_account_daily"] {
        if let Some(map) = value.get_mut(field).and_then(serde_json::Value::as_object_mut) {
            let old = std::mem::take(map);
            for (name, entry) in old { map.insert(scoped(&name), entry); }
        }
    }
    if let Some(sticky) = value.get_mut("sticky").and_then(serde_json::Value::as_object_mut) {
    ...
    for (field, pool_field) in [
        ...
        if let Some(name) = value.get(field).and_then(|v| v.as_str()).map(ToOwned::to_owned) {
            let scoped_name = scoped(&name);
            if codex_names.contains(&name) {
                let root = value.as_object_mut().context("state root must be a JSON object")?;
                let pools = root.entry(pool_field).or_insert_with(|| serde_json::json!({}));
                pools.as_object_mut().context("pool state must be a JSON object")?
    ...
    if let Some(requests) = value.get_mut("recent_requests").and_then(serde_json::Value::as_array_mut) {
...
pub fn config_needs_migration(path: &Path) -> Result<bool> {
    ...
    Ok(value.get("schema_version").and_then(toml::Value::as_integer) != Some(CONFIG_SCHEMA_VERSION as i64))
}
...
"#).unwrap();
        ...
        assert_eq!(config.pools.codex.overflow.account.as_deref(), Some("codex/api-overflow"));
        assert!(config.accounts.iter().any(|a| a.name == "codex/api-overflow" && a.provider == Provider::OpenAIApi));
...
"#).unwrap();
        ...
        let legacy = migrated.get("accounts").and_then(toml::Value::as_array).unwrap();
        assert_eq!(legacy.len(), 1);
        assert_eq!(legacy[0].get("name").and_then(toml::Value::as_str), Some("groq"));
        assert!(migrated.get("server").and_then(|v| v.get("classifier_account")).is_none());
        assert_eq!(migrated.get("classifier").and_then(|v| v.get("model"))
            .and_then(toml::Value::as_str), Some("qwen-test"));
