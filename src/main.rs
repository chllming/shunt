...
fn preflight_kill() {
    ...
    let Ok(old_pid) = content.trim().parse::<u32>() else {
        return;
    };
...
fn load_addrs(config_override: Option<&Path>) -> Vec<(String, String)> {
        ...
        .any(|a| matches!(a.provider, Provider::Anthropic | Provider::AnthropicApi))
    {
        out.push((
            "claude".into(),
            format!("http://{host}:{}", cfg.pools.claude.port),
        ));
    }
    if cfg
        .accounts
        ...
        .any(|a| matches!(a.provider, Provider::OpenAI | Provider::OpenAIApi))
    {
        out.push((
            "codex".into(),
            format!("http://{host}:{}", cfg.pools.codex.port),
        ));
    }
    use std::collections::BTreeSet;
    let providers: BTreeSet<String> = cfg
        .accounts
        .iter()
        .filter(|a| {
            !matches!(
