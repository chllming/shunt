    ...
    match stream.next().await {
        Some(Ok(Message::Text(text))) => {
            let frame: RelayFrame =
                serde_json::from_str(&text).context("Invalid relay response")?;
            match frame {
...
async fn handle_request(
    ...
    let send_err = |msg: &str| {
        let frame = serde_json::to_string(&ClientFrame::ResErr {
            id: &id,
            message: msg,
        })
        .unwrap();
    ...
    let body_bytes = match B64.decode(&body_b64) {
        Ok(b) => b,
        Err(e) => {
            send_err(&format!("base64 decode: {e}")).await;
            return;
        }
    };
    ...
    let method = match method.parse::<reqwest::Method>() {
        Ok(m) => m,
        Err(e) => {
            send_err(&format!("bad method: {e}")).await;
            return;
        }
    };
    ...
    let resp = match req.send().await {
        Ok(r) => r,
        Err(e) => {
            send_err(&format!("local request failed: {e}")).await;
            return;
        }
    };
