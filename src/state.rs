...
impl QuotaWindow {
    ...
    pub fn window_expires_ms(&self) -> Option<u64> {
        if self.window_start_ms == 0 {
            ...
            Some(self.window_start_ms + WINDOW_MS)
        }
    }
}
...
impl StateStore {
    ...
    pub fn is_exhausted(&self, name: &str) -> bool {
        let now_secs = SystemTime::now()
            ...
            .as_secs();
        let data = self.inner.lock();
        let Some(rl) = data.rate_limits.get(name) else {
            return false;
        };
    ...
    pub fn routing_snapshot(&self) -> RoutingSnapshot {
        ...
        let burst_counts: HashMap<String, usize> = all_names
            .iter()
            .map(|name| {
                let count = data
                    .burst_windows
        ...
        let accounts: HashMap<String, AccountRoutingData> = all_names
            .iter()
            .map(|name| {
                let acc = data.accounts.get(name);
                let available = acc
                    .map(|a| !a.disabled && !a.auth_failed && now_ms >= a.cooldown_until_ms)
                    .unwrap_or(true);
                ...
                let (util_5h, reset_5h, util_7d, reset_7d, exhausted) =
                    if let Some(rl) = data.rate_limits.get(name) {
                        ...
                        let u5 = if r5.is_some() {
                            rl.utilization_5h.unwrap_or(0.0)
                        } else {
                            0.0
                        };
                        let u7 = if r7.is_some() {
    ...
    pub fn get_pinned(&self) -> Option<String> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.pinned_account.clone()
        } else {
            data.pinned_by_pool.get(&self.namespace).cloned().flatten()
        }
    }
    ...
    pub fn set_pinned(&self, name: Option<String>) {
        {
            ...
            if self.namespace == "claude" {
                ...
                data.pinned_by_pool.insert(self.namespace.clone(), name);
            }
        }
        self.persist();
    }
    ...
    pub fn get_last_used(&self) -> Option<String> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.last_used_account.clone()
        } else {
            data.last_used_by_pool.get(&self.namespace).cloned()
        }
    }
    ...
    pub fn set_last_used(&self, name: &str) {
        {
            ...
            if self.namespace == "claude" {
                ...
                data.last_used_by_pool
                    .insert(self.namespace.clone(), name.to_owned());
            }
        }
        self.persist();
    }
    ...
    pub fn get_model_override(&self) -> Option<String> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.model_override.clone()
        } else {
            data.model_override_by_pool.get(&self.namespace).cloned()
        }
    }
    ...
    pub fn set_model_override(&self, model: String) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.model_override = Some(model);
        } else {
            data.model_override_by_pool
                .insert(self.namespace.clone(), model);
        }
    }
    ...
    pub fn clear_model_override(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.model_override = None;
        } else {
            data.model_override_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn get_routing_strategy(&self) -> Option<RoutingStrategy> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.routing_strategy_override
        } else {
            data.routing_strategy_by_pool.get(&self.namespace).copied()
        }
    }
    ...
    pub fn set_routing_strategy(&self, strategy: RoutingStrategy) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.routing_strategy_override = Some(strategy);
        } else {
            data.routing_strategy_by_pool
                .insert(self.namespace.clone(), strategy);
        }
    }
    ...
    pub fn clear_routing_strategy(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.routing_strategy_override = None;
        } else {
            data.routing_strategy_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn get_burst_rpm_limit_override(&self) -> Option<u32> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.burst_rpm_limit_override
        } else {
            data.burst_rpm_limit_by_pool.get(&self.namespace).copied()
        }
    }
    ...
    pub fn set_burst_rpm_limit_override(&self, limit: u32) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.burst_rpm_limit_override = Some(limit);
        } else {
            data.burst_rpm_limit_by_pool
                .insert(self.namespace.clone(), limit);
        }
    }
    ...
    pub fn clear_burst_rpm_limit_override(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.burst_rpm_limit_override = None;
        } else {
            data.burst_rpm_limit_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn get_fallback_model_override(&self) -> Option<Option<String>> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.fallback_model_override.clone()
        } else {
            data.fallback_model_by_pool.get(&self.namespace).cloned()
        }
    }
    ...
    pub fn set_fallback_model_override(&self, model: Option<String>) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.fallback_model_override = Some(model);
        } else {
            data.fallback_model_by_pool
                .insert(self.namespace.clone(), model);
        }
    }
    ...
    pub fn clear_fallback_model_override(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.fallback_model_override = None;
        } else {
            data.fallback_model_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn get_effort_override(&self) -> Option<String> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.effort_override.clone()
        } else {
            data.effort_by_pool.get(&self.namespace).cloned()
        }
    }
    ...
    pub fn set_effort_override(&self, effort: String) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.effort_override = Some(effort);
        } else {
            data.effort_by_pool.insert(self.namespace.clone(), effort);
        }
    }
    ...
    pub fn clear_effort_override(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.effort_override = None;
        } else {
            data.effort_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn get_thinking_override(&self) -> Option<String> {
        let data = self.inner.lock();
        if self.namespace == "claude" {
            data.thinking_override.clone()
        } else {
            data.thinking_by_pool.get(&self.namespace).cloned()
        }
    }
    ...
    pub fn set_thinking_override(&self, mode: String) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.thinking_override = Some(mode);
        } else {
            data.thinking_by_pool.insert(self.namespace.clone(), mode);
        }
    }
    ...
    pub fn clear_thinking_override(&self) {
        let mut data = self.inner.lock();
        if self.namespace == "claude" {
            data.thinking_override = None;
        } else {
            data.thinking_by_pool.remove(&self.namespace);
        }
    }
    ...
    pub fn reserve_budget(
        ...
        if !estimated_usd.is_finite() || estimated_usd <= 0.0 || daily_budget_usd <= 0.0 {
            return None;
        }
        let today = today_key();
        ...
        let _disk = (self.path != Path::new("/dev/null")).then(|| self.disk_lock.lock());
        {
            ...
            let spent = data
                ...
                .map(|b| b.api_cost_usd)
                .unwrap_or(0.0);
            let reserved: f64 = data
                .budget_reservations
                .values()
                .filter(|r| r.account == account && r.day == today)
                .map(|r| r.usd)
                .sum();
            if spent + reserved + estimated_usd > daily_budget_usd + f64::EPSILON {
                return None;
            }
            data.budget_reservations.insert(
                id.clone(),
                BudgetReservation {
    ...
    pub fn savings_snapshot(&self) -> SavingsSnapshot {
        ...
        let (week_input, week_output, week_cost) = data
            .global_daily
            .iter()
...
mod tests {
    ...
    fn test_sticky_ttl_expiry() {
        ...
        assert_eq!(
            store.get_sticky(fp).as_deref(),
            Some("account1"),
            "sticky should be available immediately"
        );
        std::thread::sleep(std::time::Duration::from_millis(600));
        assert!(
            store.get_sticky(fp).is_none(),
    ...
    fn test_cooldown_blocks_availability() {
        ...
        assert!(
            !store.is_available("acc"),
            "account should be unavailable during cooldown"
        );
    }
    ...
    fn test_disable_blocks_availability() {
        ...
        assert!(
            !store.is_available("acc"),
            "disabled account must be unavailable"
        );
    }
    ...
    fn test_recent_requests_ring_buffer() {
        ...
        assert_eq!(
            snap.len(),
            MAX_RECENT,
            "buffer must not grow beyond MAX_RECENT"
        );
        // Most recent first
        assert!(
            snap[0].ts_ms > snap[snap.len() - 1].ts_ms,
    ...
    fn budget_reservation_is_durable_before_admission_returns() {
        let path =
            std::env::temp_dir().join(format!("shunt-budget-state-{}.json", uuid::Uuid::new_v4(),));
        let store = StateStore::load(&path);
        let id = store.reserve_budget("api", 1.0, 5.0).unwrap();
        let persisted: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        assert_eq!(
            persisted["budget_reservations"][id.as_str()]["account"],
            "api"
        );
    ...
    fn pool_scopes_keep_runtime_overrides_independent() {
        ...
        assert_eq!(
            claude.get_model_override().as_deref(),
            Some("claude-sonnet")
        );
    ...
    fn test_health_check_failed_round_trip() {
        let store = StateStore::new_empty();
        assert!(
            !store.is_health_check_failed("acc"),
            "fresh account should not be health-check-failed"
        );

        store.set_health_check_failed("acc");
        assert!(
            store.is_health_check_failed("acc"),
            "should be marked unhealthy after set"
        );

        store.clear_health_check_failed("acc");
        assert!(
            !store.is_health_check_failed("acc"),
            "should be cleared after clear"
        );
    }
    ...
    fn test_health_check_failed_persists() {
        ...
        assert!(
            store2.is_health_check_failed("acc"),
        ...
        assert_eq!(
            last, 0,
            "last_health_check_ms is ephemeral, should be 0 after reload"
        );
        assert_eq!(
            failures, 0,
            "health_check_failures is ephemeral, should be 0 after reload"
        );
    ...
    fn test_burst_window_tracking() {
        ...
        assert!(
            data.is_none() || data.unwrap().burst_request_count == 0,
        ...
        let data = snap
            .accounts
            .get("acc")
            .expect("acc should exist in snapshot");
        // Should have all 8 requests (5 + 3) since they're within the 60s window
        assert_eq!(
            data.burst_request_count, 8,
            "should count all recent requests"
        );
    }
}
