...
static ADMISSION: std::sync::OnceLock<crate::limiter::AdmissionLimiter> =
    std::sync::OnceLock::new();

fn shared_admission() -> crate::limiter::AdmissionLimiter {
    ADMISSION
        .get_or_init(crate::limiter::AdmissionLimiter::new)
        .clone()
}
...
static WARM_START: std::sync::OnceLock<Arc<ParkingMutex<HashMap<String, (u64, u64)>>>> =
    std::sync::OnceLock::new();

fn shared_warm_start() -> Arc<ParkingMutex<HashMap<String, (u64, u64)>>> {
    WARM_START
        .get_or_init(|| Arc::new(ParkingMutex::new(HashMap::new())))
        .clone()
}
...
impl BudgetGuard {
    fn new(state: &StateStore, id: String) -> Self {
        Self {
            state: state.clone(),
            id: Some(id),
        }
    }
    fn handoff(mut self) -> Option<String> {
        self.id.take()
    }
}
...
impl Drop for BudgetGuard {
    fn drop(&mut self) {
        if let Some(id) = self.id.take() {
            self.state.release_budget_reservation(&id);
        }
    }
}
...
fn build_app_state_with(
    ...
    let rate_limiter = if config.server.rate_limit_rpm > 0 {
        Some(Arc::new(ParkingMutex::new(
            HashMap::<IpAddr, TokenBucket>::new(),
        )))
    } else {
        None
    };
...
pub fn create_control_app(config: Config, state: StateStore) -> anyhow::Result<Router> {
    ...
    let app = Router::new()
        ...
        .route(
            "/pools/:pool/model",
            get(pool_model_get_handler)
                .post(pool_model_set_handler)
                .delete(pool_model_clear_handler),
        )
        .route(
            "/pools/:pool/strategy",
            get(pool_strategy_get_handler)
                .post(pool_strategy_set_handler)
                .delete(pool_strategy_clear_handler),
        )
        .route("/bridge/tools/:tool", post(bridge_tool_handler))
        .route(
            "/model",
            get(model_get_handler)
                .post(model_set_handler)
                .delete(model_clear_handler),
        )
        .route(
            "/strategy",
            get(strategy_get_handler)
                .post(strategy_set_handler)
                .delete(strategy_clear_handler),
        )
        .route(
            "/burst-limit",
            get(burst_limit_get_handler)
                .post(burst_limit_set_handler)
                .delete(burst_limit_clear_handler),
        )
        .route(
            "/fallback",
            get(fallback_get_handler)
                .post(fallback_set_handler)
                .delete(fallback_clear_handler),
        )
        .route(
            "/effort",
            get(effort_get_handler)
                .post(effort_set_handler)
                .delete(effort_clear_handler),
        )
        .route(
            "/thinking",
            get(thinking_get_handler)
                .post(thinking_set_handler)
                .delete(thinking_clear_handler),
        )
...
fn scoped_pool_state(mut state: AppState, pool: &str) -> Result<AppState, Response> {
    let pool_kind = match pool {
        ...
        _ => {
            return Err((
                ...
                axum::Json(json!({"error":"unknown pool"})),
            )
                .into_response())
        }
    };
    ...
    config.accounts.retain(|account| match pool_kind {
        crate::config::PoolKind::Claude => matches!(
            account.provider,
            Provider::Anthropic | Provider::AnthropicApi
        ),
        crate::config::PoolKind::Codex => {
            matches!(account.provider, Provider::OpenAI | Provider::OpenAIApi)
        }
...
async fn pool_model_get_handler(
    AxumPath(pool): AxumPath<String>,
    State(state): State<AppState>,
) -> Response {
    match scoped_pool_state(state, &pool) {
...
async fn pool_model_clear_handler(
    AxumPath(pool): AxumPath<String>,
    State(state): State<AppState>,
) -> Response {
    match scoped_pool_state(state, &pool) {
...
async fn pool_strategy_get_handler(
    AxumPath(pool): AxumPath<String>,
    State(state): State<AppState>,
) -> Response {
    match scoped_pool_state(state, &pool) {
...
async fn pool_strategy_clear_handler(
    AxumPath(pool): AxumPath<String>,
    State(state): State<AppState>,
) -> Response {
    match scoped_pool_state(state, &pool) {
...
async fn bridge_tool_handler(
    ...
    if !authorized {
        return (
            ...
            axum::Json(json!({"error":"invalid bridge token"})),
        )
            .into_response();
    }
    let caller = body
    ...
    match crate::bridge::dispatch_tool(
        &tool,
        caller,
        arguments,
        depth,
        Some(&state.config.config_file),
    )
    .await
    {
...
async fn status_handler(State(s): State<AppState>) -> impl IntoResponse {
    ...
    let accounts: Vec<_> = s
        ...
        .map(|a| {
            ...
            let admission_json = adm.map(|(in_flight, limit, recent_429)| {
                json!({
                    "in_flight": in_flight,
                    "limit": limit,
...
async fn model_set_handler(
    ...
    let Some(model) = body["model"].as_str() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing model field" })),
        )
            .into_response();
    };
...
async fn strategy_set_handler(
    ...
    let Some(name) = body["strategy"].as_str() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing strategy field" })),
        )
            .into_response();
    };
    let Some(strategy) = crate::config::RoutingStrategy::from_str(name) else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": format!("unknown strategy '{name}'") })),
        )
            .into_response();
    };
...
async fn burst_limit_get_handler(State(s): State<AppState>) -> impl IntoResponse {
    let (limit, source) = match s.state.get_burst_rpm_limit_override() {
        Some(l) => (l, "override"),
        None => (
            s.config.server.burst_rpm_limit,
            if s.config.server.burst_rpm_limit == 10 {
                "default"
            } else {
                "config"
            },
...
async fn burst_limit_set_handler(
    ...
    let Some(limit) = body["burst_rpm_limit"].as_u64() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing burst_rpm_limit field (integer)" })),
        )
            .into_response();
    };
...
async fn burst_limit_clear_handler(State(s): State<AppState>) -> impl IntoResponse {
    ...
    axum::Json(
        json!({ "burst_rpm_limit": limit, "source": if limit == 10 { "default" } else { "config" } }),
    )
}
...
async fn fallback_get_handler(State(s): State<AppState>) -> impl IntoResponse {
    match s.state.get_fallback_model_override() {
        ...
        Some(None) => {
            axum::Json(json!({ "fallback_model": null, "source": "override", "disabled": true }))
        }
...
async fn fallback_set_handler(
    ...
    if body["fallback_model"].is_null()
        || body.get("disabled").and_then(|v| v.as_bool()) == Some(true)
    {
        s.state.set_fallback_model_override(None);
        info!("fallback model explicitly disabled");
        return axum::Json(
            json!({ "fallback_model": null, "source": "override", "disabled": true }),
        )
        .into_response();
    }
    let Some(model) = body["fallback_model"].as_str() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing fallback_model field" })),
        )
            .into_response();
    };
...
async fn effort_set_handler(
    ...
    let Some(effort) = body["effort"].as_str() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing effort string field" })),
        )
            .into_response();
    };
    ...
    if !valid.contains(&effort) {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "effort must be one of: low, medium, high, max" })),
        )
            .into_response();
    }
...
async fn thinking_set_handler(
    ...
    let Some(mode) = body["thinking"].as_str() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing thinking string field" })),
        )
            .into_response();
    };
    ...
    if !valid.contains(&mode) {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "thinking must be one of: adaptive, disabled" })),
        )
            .into_response();
    }
...
async fn alerts_set_handler(
    ...
    let Some(muted) = body["muted"].as_bool() else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "missing muted bool field" })),
        )
            .into_response();
    };
...
async fn claude_classifier_response(s: &AppState, request_body: &[u8]) -> Option<Response> {
    ...
    let start = request_body
        .len()
        .saturating_sub(CLASSIFIER_MAX_TRANSCRIPT_CHARS);
    let transcript = String::from_utf8_lossy(&request_body[start..]);
    let prompt = format!(
    ...
    if streaming {
        let events = [
            (
                "message_start",
                json!({"type":"message_start","message":{"id":id,"type":"message","role":"assistant","model":classifier_model,"content":[],"stop_reason":null,"usage":{"input_tokens":0,"output_tokens":0}}}),
            ),
            (
                "content_block_start",
            ...
            (
                "content_block_delta",
                json!({"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":verdict}}),
            ),
            (
                "content_block_stop",
        ...
        Some(
            Response::builder()
                ...
                .expect("classifier SSE response"),
        )
    } else {
        Some(
            axum::Json(json!({
...
async fn codex_handler(State(s): State<AppState>, req: Request) -> Result<Response, ProxyError> {
        ...
        if let Some((bytes, content_type, expires)) = s.codex_models_cache.lock().clone() {
            if expires > now_ms() {
                return Ok(Response::builder()
                    ...
                    .header("x-shunt-cache", "hit")
                    .body(axum::body::Body::from(bytes))
                    .expect("cached models response"));
            }
        }
    }
    let mut body = axum::body::to_bytes(req.into_body(), MAX_REQUEST_BODY)
        .await
        .map_err(|_| ProxyError::BodyRead)?;
    let mut model = serde_json::from_slice::<serde_json::Value>(&body)
        .ok()
        .and_then(|v| {
            v.get("model")
                .and_then(|m| m.as_str())
    ...
    if model == "codex-auto-review" {
        ...
        if local_configured {
            if let Some(response) = codex_guardian_response(&s, &body).await {
                return Ok(response);
            }
            if s.config.classifier.fail_closed {
    ...
    let (affinity_account, strict_bound) = {
        ...
        let strict = strict_key
            .as_ref()
            .and_then(|key| map.get(key).map(|(account, _)| account.clone()));
        let strict_bound = strict.is_some();
        let account = strict.or_else(|| {
            soft_keys
                .iter()
                .find_map(|key| map.get(key).map(|(account, _)| account.clone()))
        });
    ...
    loop {
        ...
        let selected = bound_account.or_else(|| {
            ...
            if let Some(name) = overflow_name {
            ...
            router::pick_account(
                ...
                soft_fp,
                &excluded,
                s.config.server.sticky_ttl_ms,
                s.config.server.expiry_soon_secs,
                s.config.server.routing_strategy,
                s.config.server.burst_rpm_limit,
                None,
            )
            .filter(|a| {
                matches!(a.provider, Provider::OpenAI | Provider::OpenAIApi)
                    && (!backend_only || a.provider == Provider::OpenAI)
            })
            .or_else(|| {
                if backend_only {
            ...
            if path.ends_with("/responses") {
                if let Some(fallback) = s.config.pools.codex.fallback_models.get(fallback_index) {
                    if let Ok(mut value) = serde_json::from_slice::<serde_json::Value>(&body) {
                        ...
                        body = Bytes::from(
                            serde_json::to_vec(&value).map_err(|_| ProxyError::BodyRead)?,
                        );
        ...
        if account.provider == Provider::OpenAIApi {
            ...
            if method == "POST" {
                let mut value: serde_json::Value =
                    serde_json::from_slice(&body).map_err(|_| ProxyError::BodyRead)?;
                let requested = value
            ...
            let Some(estimated) =
                crate::pricing::strict_api_cost_usd(&model, input_upper_bound, cap)
            else {
                warn!(%model, "rejecting Codex API overflow for unpriced model");
                return Err(ProxyError::AllAccountsUnavailable(Some(60)));
            };
            reservation = s.state.reserve_budget(
                &account_name,
                estimated,
                s.config.api_overflow.daily_budget_usd,
            );
        ...
        let mut response = match response {
            ...
            Err(error) => {
                if let Some(id) = reservation.as_deref() {
                    s.state.release_budget_reservation(id);
                }
        ...
        if matches!(status, 401 | 429 | 500 | 502 | 503 | 504) {
            ...
            if status == 401 {
                    ...
                    if let Some(oauth) = credential.as_oauth() {
                        ...
                        .await
                        {
                            s.credentials
                                .write()
                                .await
                                .insert(account_name.clone(), Credential::Oauth(fresh.clone()));
                ...
                s.state.set_cooldown(
                    &account_name,
                    parse_retry_after_ms(response.headers()).unwrap_or(5_000),
                );
            }
            if strict_bound {
                return Ok(response);
        ...
        if method == "GET" && path.ends_with("/models") && status == 200 {
            ...
            let bytes = axum::body::to_bytes(response_body, 16 * 1024 * 1024)
                .await
        ...
        let expires = now_ms().saturating_add(s.config.server.sticky_ttl_ms);
        {
            ...
            for key in &soft_keys {
                map.insert(key.clone(), (account_name.clone(), expires));
            }
...
async fn proxy_handler(State(s): State<AppState>, req: Request) -> Result<Response, ProxyError> {
    ...
    let (mut body_bytes, model) = if let Ok(mut val) =
            ...
            if let Some(path) = s.config.server.classifier_system_prompt_path.as_deref() {
                match std::fs::read_to_string(path) {
                    ...
                    Ok(_) => {
                        warn!(
                            path,
                            "classifier_system_prompt_path is empty — keeping Claude Code's prompt"
                        );
                    }
                    Err(e) => {
    ...
    if is_classifier
        ...
        && s.config.classifier.model.is_some()
    {
        if let Some(response) = claude_classifier_response(&s, &body_bytes).await {
            return Ok(response);
        }
    }
    ...
    let api_lane_usable = |tried: &HashSet<String>| -> bool {
        let Some(name) = api_lane_name.as_deref() else {
            return false;
        };
        if tried.contains(name) {
            return false;
        }
        if s.state.account_spend_today_usd(name) + s.state.reserved_today_usd(name)
            >= ov.daily_budget_usd
        {
            return false;
        }
        s.config
            .accounts
            .iter()
            .any(|a| a.name == name && a.credential.is_some())
    ...
    loop {
        ...
        let effective_burst_rpm = s
            .state
            .get_burst_rpm_limit_override()
        ...
        let selected = if let Some(lane) = classifier_lane {
                    ...
                    s.config
                        ...
                        .and_then(|fb| {
                            s.config
                                .accounts
                                .iter()
                                .find(|a| a.name == fb && !tried.contains(&a.name))
                        })
                })
        } else {
            ...
            if let Some(name) = api_lane_name.as_deref() {
                excluded.insert(name.to_owned());
            }
        ...
        let account = match selected {
            ...
            None => {
                ...
                if !saturated.is_empty() && now_ms() + 250 <= wait_deadline_ms {
                ...
                let soonest_ms = s
                    ...
                    .filter_map(|a| {
                        ...
                        if st.disabled {
                            return None;
                        } // auth_failed or permanently off
                        if st.cooldown_until_ms > now {
                            Some(st.cooldown_until_ms)
                        } else {
                            None
                        }
                    })
                ...
                match soonest_ms {
                    Some(wake_ms) if wake_ms <= wait_deadline_ms => {
                        ...
                        warn!(
                            wait_ms,
                            "all accounts cooling — waiting for next available account"
                        );
        ...
        let acct_is_anthropic =
            account.provider.wire_protocol() == crate::provider::WireProtocol::Anthropic;
        ...
        if api_lane_name.as_deref() == Some(account_name.as_str()) {
            if let Ok(mut value) = serde_json::from_slice::<serde_json::Value>(&fwd_body) {
                let output_field = if value.get("max_output_tokens").is_some() {
                    "max_output_tokens"
                } else {
                    "max_tokens"
                };
                let requested = value
                    .get(output_field)
                    .and_then(|v| v.as_u64())
                    .unwrap_or(ov.max_output_tokens);
                value[output_field] = serde_json::Value::from(requested.min(ov.max_output_tokens));
                fwd_body =
                    Bytes::from(serde_json::to_vec(&value).map_err(|_| ProxyError::BodyRead)?);
            }
            ...
            let Some(reservation) =
                s.state
                    .reserve_budget(&account_name, estimated, ov.daily_budget_usd)
            else {
        ...
        match response.status().as_u16() {
            ...
            429 => {
                ...
                let retry_after_ms = parse_retry_after_ms(response.headers()).map(|ms| {
                    if is_exhausted {
                ...
                let cooldown_ms =
                    compute_429_cooldown_ms(info.as_ref(), retry_after_ms, is_exhausted);
                    ...
                    .unwrap_or(false) =>
            {
                ...
                let raw = axum::body::to_bytes(body, MAX_REQUEST_BODY)
                    .await
                    .unwrap_or_default();
                ...
                if is_long_ctx {
                    ...
                    if let Ok(mut val) = serde_json::from_slice::<serde_json::Value>(&body_bytes) {
                        if normalize_model_suffix(&mut val) {
                            body_bytes = Bytes::from(
                                serde_json::to_vec(&val).unwrap_or_else(|_| body_bytes.to_vec()),
                            );
                        }
                    }
                    stripped_1m = true;
...
pub async fn prefetch_rate_limits(
    ...
    for account in &config.accounts {
        ...
        let resp =
            prefetch_send(&client, &url, &account.provider, cred.bearer_token(), &body).await;

        let r = match resp {
            Ok(r) => r,
            Err(e) => {
                tracing::warn!(account = %account.name, "prefetch failed: {e}");
                continue;
            }
        };
        ...
        if r.status() == reqwest::StatusCode::UNAUTHORIZED {
            ...
            live_creds
                .write()
                .await
                .insert(account.name.clone(), Credential::Oauth(fresh.clone()));
...
async fn auth_probe_get(
    ...
    let resp = match do_get(cred.bearer_token()).send().await {
        Ok(r) => r,
        Err(e) => {
            tracing::warn!(account = %account.name, "auth probe failed: {e}");
            return;
        }
    };
...
fn access_token_expires_soon(cred: &crate::oauth::OAuthCredential, threshold_mins: u64) -> bool {
    let now_ms = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis() as u64;
    let exp_ms = crate::oauth::jwt_exp_ms(&cred.access_token).unwrap_or(cred.expires_at);
...
async fn sync_live_creds_from_auth_json(account_name: &str, live_creds: &LiveCredentials) {
    let Some(from_file) = crate::oauth::read_codex_credentials() else {
        return;
    };
    let current_exp = live_creds
        .read()
        .await
        .get(account_name)
    ...
    if from_file.expires_at > current_exp {
        ...
        live_creds
            .write()
            .await
            .insert(account_name.to_owned(), Credential::Oauth(from_file));
    }
}
        ...
        match self {
            ...
            other => {
                let (status, msg) = match other {
                    ...
                    ProxyError::RateLimited | ProxyError::AllAccountsUnavailable(_) => {
                        unreachable!()
                    }
                };
                (
...
pub async fn recovery_watcher(
    ...
    loop {
        ...
        for name in &failed {
            ...
            let Some(oauth_cred) = cred.as_oauth().cloned() else {
                continue;
            };
            ...
            let result =
                tokio::time::timeout(Duration::from_secs(20), provider.refresh_token(&oauth_cred))
                    .await;
                ...
                Ok(Err(e)) => {
                    ...
                    if !state.get_alerts_muted() {
                        notify(
                            ...
                            &format!(
                                "Account '{name}' needs re-authorization. Run `shunt add-account`."
                            ),
                            "Basso",
                        );
                    }
...
pub async fn health_check_loop(
    ...
    loop {
                ...
                None => {
                    ...
                    if let Some(probe_path) = account.provider.auth_probe_get_path() {
                        ...
                        match client.get(&url).send().await {
                            ...
                            Ok(r) => {
                                let count = state
                                    .record_health_check_failure(&account.name, FAILURE_THRESHOLD);
                ...
                match prefetch_send(&client, &url, &account.provider, token, &body).await {
                    Ok(r) => {
                        ...
                        if status == reqwest::StatusCode::UNAUTHORIZED {
                                ...
                                match account.provider.refresh_token(oauth_cred).await {
                                    Ok(fresh) => {
                                        ...
                                        live_creds
                                            .write()
                                            .await
                                            .insert(account.name.clone(), Credential::Oauth(fresh));
                                        state.clear_auth_failed(&account.name);
                                        if state.is_health_check_failed(&account.name) {
                                            state.clear_health_check_failed(&account.name);
                                        }
                            ...
                            let count =
                                state.record_health_check_failure(&account.name, FAILURE_THRESHOLD);
        ...
        tokio::time::sleep(std::time::Duration::from_secs(
            config.server.health_check_interval_secs,
        ))
        .await;
    }
}
...
pub async fn cooldown_watcher(
    ...
    loop {
        ...
        for account in &config.accounts {
            let Some(st) = states.get(&account.name) else {
                continue;
            };
            if st.disabled {
                continue;
            } // auth_failed or permanently disabled
            let cdl = st.cooldown_until_ms;

            if cdl > 0 && cdl <= now {
                // Cooldown expired — skip if we already handled this exact deadline
                let handled = last_resumed
                    .get(&account.name)
                    .map(|&t| t >= cdl)
                    .unwrap_or(false);
                if !handled {
...
use crate::translate::{
    translate_anthropic_req_to_chatgpt, translate_anthropic_req_to_openai,
    translate_anthropic_stream, translate_from_anthropic, translate_response_anthropic_to_openai,
    translate_response_chatgpt_to_anthropic, translate_response_openai_to_anthropic,
    translate_to_anthropic, uuid_v4,
};
...
async fn openai_compat_handler(
    ...
    let openai_body: serde_json::Value = serde_json::from_slice(&body_bytes).unwrap_or(json!({}));
    ...
    if stream {
        ...
        let anthropic_resp: serde_json::Value =
            resp.json().await.map_err(|_| ProxyError::Upstream)?;
...
fn overage_exhausted_reset(info: &RateLimitInfo) -> Option<u64> {
    ...
    let overage_blocked = matches!(
        info.overage_status.as_deref(),
        Some("rejected") | Some("disabled")
    );
    if !overage_blocked {
        return None;
    }
...
fn near_cap_7d_warning(info: &RateLimitInfo) -> bool {
    let util_high = info
        .utilization_7d
        .map(|u| u >= NEAR_CAP_7D_UTILIZATION)
        .unwrap_or(false);
    if !util_high {
        return false;
    }
...
fn bound_classifier_transcript(val: &mut serde_json::Value, max_chars: usize) -> bool {
    ...
    let total: usize = messages.iter().map(&text_len).sum();
...
mod tests {
    ...
    fn make_info(
        status_5h: Option<&str>,
        reset_5h: Option<u64>,
        status_7d: Option<&str>,
        reset_7d: Option<u64>,
    ) -> RateLimitInfo {
        RateLimitInfo {
    ...
    fn burst_429_uses_retry_after_within_short_cap() {
        ...
        assert!(
            (8_000..8_000 + JITTER_MAX_MS).contains(&cd),
            "burst 429 honors short retry_after (+jitter): {cd}"
        );
    }
    ...
    fn burst_429_caps_at_15s() {
        ...
        assert!(
            (15_000..15_000 + JITTER_MAX_MS).contains(&cd),
            "burst cooldown caps at 15s (+jitter): {cd}"
        );
    }
    ...
    fn burst_429_default_when_no_retry_after() {
        ...
        assert!(
            (5_000..5_000 + JITTER_MAX_MS).contains(&cd),
            "burst cooldown defaults to 5s (+jitter): {cd}"
        );
    }
    ...
    fn burst_429_floor_prevents_thrash() {
        ...
        assert!(
            (2_000..2_000 + JITTER_MAX_MS).contains(&cd),
            "burst cooldown floors at 2s (+jitter): {cd}"
        );
    }
    ...
    fn exhausted_429_uses_reset_derived_cooldown() {
        ...
        assert!(
            cd > 60_000 && cd <= 120_500,
            "exhausted should use reset-derived cooldown (~120s), got {cd}"
        );
    }
    ...
    fn is_exhausted_requires_status_and_future_reset() {
        ...
        assert!(is_exhausted_response(Some(&make_info(
            Some("exhausted"),
            Some(future),
            None,
            None
    ...
    fn pin_model_is_noop_when_already_classifier_model() {
        ...
        assert!(
            !pin_model_to_classifier(&mut v),
            "already on classifier model → no change"
        );
        assert_eq!(v["model"], json!(CLASSIFIER_MODEL));
    }
    ...
    fn bound_classifier_transcript_keeps_tail_when_oversized() {
        ...
        assert!(
            out.contains("<block> immediately."),
            "tail (the action) preserved"
        );
    ...
    fn bound_classifier_transcript_trims_across_many_messages() {
        ...
        let before: usize = v["messages"]
            ...
            .sum();
        assert!(
            bound_classifier_transcript(&mut v, 2000),
            "should trim; before={before}"
        );
        let after: usize = v["messages"]
            .as_array()
            .unwrap()
            .iter()
            .filter_map(|m| m["content"].as_str())
        ...
        let last = v["messages"].as_array().unwrap().last().unwrap()["content"]
            ...
            .unwrap();
        assert!(
            last.contains("git push origin main"),
            "action preserved: {last}"
        );
    }
    ...
    fn custom_classifier_prompt_file_declares_the_verdict_contract() {
        ...
        assert!(
            prompt.contains("<block>no</block>"),
            "prompt must show the allow token"
        );
        assert!(
            prompt.contains("<block>yes</block>"),
            "prompt must show the block token"
        );
    }
    ...
    fn strip_removes_effort_and_thinking_for_simple_model() {
        ...
        assert!(
            v["output_config"].get("effort").is_none(),
            "output_config.effort dropped"
        );
        ...
        assert!(
            !betas.iter().any(|b| b == "interleaved-thinking-2025-05-14"),
            "thinking beta dropped"
        );
        assert!(
            betas.iter().any(|b| b == "some-other-beta"),
            "unrelated beta preserved"
        );
    ...
    fn strip_drops_output_config_when_only_effort() {
        ...
        assert!(
            ...
            "empty output_config removed entirely"
        );
    }
    ...
    fn strip_is_noop_when_no_unsupported_params() {
        ...
        assert!(
            !strip_unsupported_params_for_simple_model(&mut v),
            "nothing to strip → no change"
        );
    }
    ...
    fn strip_beta_header_removes_thinking_effort_flags() {
        ...
        h.insert(
            "anthropic-beta",
            "interleaved-thinking-2025-05-14,oauth-2025-04-20,effort-2025"
                .parse()
                .unwrap(),
        );
    ...
    fn warm_start_active_by_request_count_then_graduates() {
        ...
        assert!(
            warm_start_active(&warm, Some("t1"), 2, 0),
            "1st request warms"
        );
        assert!(
            warm_start_active(&warm, Some("t1"), 2, 0),
    ...
    fn warm_start_active_by_age_window() {
        ...
        assert!(
            warm_start_active(&warm, Some("t2"), 0, 60_000),
            "within age window warms"
        );
    }
    ...
    fn warm_start_never_for_untraced_requests() {
        let warm = ParkingMutex::new(HashMap::new());
        assert!(
            !warm_start_active(&warm, None, 5, 60_000),
            "no trace → never warm-start"
        );
    }
    ...
    fn burst_jitter_stays_in_bounds() {
        for _ in 0..1000 {
            assert!(
                burst_cooldown_jitter_ms() < 3_000,
                "jitter must stay under 3s"
            );
        }
    }
    ...
    fn burst_cooldown_short_and_bounded_with_jitter() {
        ...
        for _ in 0..200 {
            ...
            assert!(
                (2_000..=18_000).contains(&ms),
                "burst cooldown {ms} out of expected 2-18s band"
            );
        }
    }
    ...
    fn strip_beta_header_removes_header_when_all_flags_stripped() {
        ...
        assert!(
            ...
            "header removed when nothing left"
        );
    }
    ...
    fn strip_long_context_beta_removes_flag_preserves_others() {
        ...
        h.insert(
            "anthropic-beta",
            "context-1m-2025-08-07,oauth-2025-04-20,claude-code-20250219"
                .parse()
                .unwrap(),
        );
    ...
    fn strip_long_context_beta_removes_header_when_only_flag() {
        ...
        assert!(
            h.get("anthropic-beta").is_none(),
            "header removed when 1M was the only flag"
        );
    }
    ...
    fn simple_model_rewrite_drops_both_effort_and_1m_beta() {
        ...
        h.insert(
            "anthropic-beta",
            "context-1m-2025-08-07,interleaved-thinking-2025-05-14,oauth-2025-04-20"
                .parse()
                .unwrap(),
        );
        strip_beta_header_for_simple_model(&mut h);
        ...
        assert!(
            ...
            "oauth preserved for subscription recognition"
        );
    }
}
