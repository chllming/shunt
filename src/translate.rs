...
pub fn translate_anthropic_req_to_openai(body: Value, target_model: &str) -> Value {
                    ...
                    for b in blocks {
                        if b["type"] == "tool_result" {
                            let content = b["content"]
                                ...
                                .map(|s| s.to_owned())
                                .unwrap_or_else(|| {
                                    serde_json::to_string(&b["content"]).unwrap_or_default()
                                });
                            messages.push(json!({
                                "role": "tool",
    ...
    if let Some(sp) = body.get("stop_sequences") {
        req["stop"] = sp.clone();
    }

    // Anthropic tools → OpenAI tools.
    if let Some(tools) = body["tools"].as_array() {
        let oai: Vec<Value> = tools
            .iter()
            .map(|t| {
                json!({
                    "type": "function",
...
pub fn translate_openai_resp_to_anthropic(body: Value, model: &str) -> Value {
    ...
    let stop_reason =
        openai_finish_to_anthropic(choice["finish_reason"].as_str().unwrap_or("stop"));
        ...
        while let Some(chunk) = byte_stream.next().await {
            ...
            while let Some(nl) = buf.find('\n') {
                ...
                if let Some(fr) = finish {
                    ...
                    for (my_idx, _, _) in tool_blocks.values() {
                        yield Ok(Bytes::from(format!(
                            "event: content_block_stop\ndata: {}\n\n",
