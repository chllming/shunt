    ...
    pub fn parse_rate_limits(&self, headers: &HeaderMap) -> Option<RateLimitInfo> {
        ...
        match self {
            Provider::Anthropic | Provider::AnthropicApi => {
                parse_anthropic_rate_limits(headers, now_ms)
            }
    ...
    pub fn read_local_credentials(&self) -> Option<Credential> {
        match self.auth_kind() {
            AuthKind::OAuth => match self {
                ...
                Provider::OpenAI => crate::oauth::read_codex_credentials().map(Credential::Oauth),
                _ => None,
            },
            AuthKind::ApiKey => {
...
fn parse_codex_rate_limits(headers: &HeaderMap, now_ms: u64) -> Option<RateLimitInfo> {
    ...
    let primary =
        number(headers, "x-codex-primary-used-percent").map(|v| (v / 100.0).clamp(0.0, 1.0));
    let secondary =
        number(headers, "x-codex-secondary-used-percent").map(|v| (v / 100.0).clamp(0.0, 1.0));
...
fn parse_anthropic_rate_limits(headers: &HeaderMap, now_ms: u64) -> Option<RateLimitInfo> {
    ...
    Some(RateLimitInfo {
        ...
        overage_disabled_reason: hdr_str(
            headers,
...
mod tests {
    ...
    fn test_wire_protocol() {
        ...
        assert_eq!(
            Provider::OpenAI.wire_protocol(),
            WireProtocol::OpenAIResponses
        );
        assert_eq!(
            Provider::OpenAIApi.wire_protocol(),
    ...
    fn test_anthropic_api_from_str_and_kind() {
        ...
        assert_eq!(
            Provider::AnthropicApi.wire_protocol(),
            WireProtocol::Anthropic
        );
        assert!(Provider::AnthropicApi.accepts_claude_models());
        assert_eq!(
            Provider::AnthropicApi.default_upstream_url(),
    ...
    fn test_anthropic_api_injects_x_api_key_not_bearer() {
        ...
        Provider::AnthropicApi
            .inject_auth_headers(&mut headers, "sk-ant-test123")
            .unwrap();

        assert_eq!(headers.get("x-api-key").unwrap(), "sk-ant-test123");
        assert!(
            headers.get("authorization").is_none(),
            "must not send Bearer for Console API key"
        );
    ...
    fn test_anthropic_oauth_still_uses_bearer() {
        let mut headers = reqwest::header::HeaderMap::new();
        Provider::Anthropic
            .inject_auth_headers(&mut headers, "oauth-token")
            .unwrap();
    ...
    fn test_openai_utilization_computation() {
        ...
        assert!(
            (util - 0.25).abs() < 0.001,
            "utilization should be 0.25 (75k/100k remaining)"
        );
