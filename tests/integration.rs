...
impl Captures {
    fn push(&self, r: CapturedRequest) {
        self.inner.lock().unwrap().push(r);
    }
    fn get(&self, i: usize) -> CapturedRequest {
        self.inner.lock().unwrap()[i].clone()
    }
    fn len(&self) -> usize {
...
async fn setup(
    streaming: bool,
    upstream_status: u16,
) -> (TestServer, TestServer, Captures, Client) {
    let caps = Captures::default();
    let upstream =
        TestServer::start(make_mock_upstream(caps.clone(), streaming, upstream_status)).await;
...
async fn test_status() {
    ...
    assert_eq!(resp.status(), 200);
    assert_eq!(
        resp.json::<serde_json::Value>().await.unwrap()["accounts"][0]["name"],
        "test"
    );
}
...
async fn test_upstream_error_returned_to_client() {
    ...
    assert_eq!(
        resp.json::<serde_json::Value>().await.unwrap()["type"],
        "error"
    );
}
...
async fn test_count_tokens_forwarded() {
    ...
    assert_eq!(resp.status(), 200);
    assert_eq!(
        resp.json::<serde_json::Value>().await.unwrap()["input_tokens"],
        99
    );
...
async fn test_status_shows_account_status() {
    ...
    assert_eq!(
        a1["status"], "cooling",
        "account1 should be cooling after 429"
    );
    assert_eq!(
        a2["status"], "available",
...
async fn test_status_response_shape() {
    ...
    assert!(
        body.as_object().unwrap().contains_key("pinned_account"),
        "top-level key 'pinned_account' must be present"
    );
    assert!(
        body.as_object().unwrap().contains_key("last_used_account"),
    ...
    assert!(
        acc.get("plan_type").is_some(),
        "account must have 'plan_type'"
    );
    assert!(
        acc.get("plan").is_none(),
        "legacy 'plan' field must be absent"
    );
...
async fn test_remote_key_auth() {
    ...
    assert_eq!(
        caps.len(),
        1,
        "only the authenticated request should reach upstream"
    );
}
...
async fn test_use_unknown_account_returns_error() {
    ...
    assert!(
        ...
        "unknown account must return error"
    );
}
...
async fn test_native_codex_responses_is_byte_transparent_and_injects_identity() {
    ...
    let cfg = Config {
        ...
        accounts: vec![AccountConfig {
            ...
            upstream_url: Some(upstream.url()),
            model: None,
        }],
    ...
    let response = Client::new()
        .post(format!(
            "{}/backend-api/codex/responses?store=false",
            proxy.url()
        ))
        .header("content-type", "application/json")
...
async fn test_codex_soft_affinity_fails_over_but_turn_state_is_strict() {
    ...
    let cfg = Config {
        ...
        accounts: vec![
            account("codex/one", "codex-one"),
            account("codex/two", "codex-two"),
        ],
        config_file: "/dev/null".into(),
        model_mapping: Default::default(),
        api_overflow: Default::default(),
    ...
    let first = Client::new()
        .post(format!("{}/backend-api/codex/responses", proxy.url()))
        .header("session-id", "soft-session")
        .json(&body)
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(
        first.headers().get("x-codex-turn-state").unwrap(),
        "turn-one"
    );
    ...
    assert_eq!(
        soft.status(),
        200,
        "soft affinity should fail over after a pre-stream 500"
    );
    assert_eq!(
        soft.headers().get("x-codex-turn-state").unwrap(),
        "turn-two"
    );
    ...
    assert_eq!(
        strict.status(),
        500,
        "known turn state must not cross accounts"
    );
...
async fn test_codex_api_overflow_reserves_budget_before_dispatch() {
    ...
    let response = Client::new()
        .post(format!("{}/backend-api/codex/responses", proxy.url()))
        .json(&json!({"model":"gpt-5.4","input":"hello","stream":true}))
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(
        caps.len(),
        0,
        "over-budget request must be rejected before upstream dispatch"
    );
}
...
async fn test_interop_anthropic_request_to_openai_account() {
    ...
    assert!(
        upstream_req.get("max_tokens").is_some(),
        "must have max_tokens"
    );
    // model must be mapped to an OpenAI model
    let sent_model = upstream_req["model"].as_str().unwrap();
    assert!(
        !sent_model.starts_with("claude-"),
        "claude-* model must be mapped to OpenAI model, got: {sent_model}"
    );
    ...
    assert!(
        caps.get(0).headers.get("anthropic-version").is_none(),
        "anthropic-version must not be forwarded to OpenAI"
    );
    ...
    assert_eq!(
        body["type"], "message",
        "response must be Anthropic message type"
    );
    assert!(
        body["content"].as_array().is_some(),
...
async fn test_interop_anthropic_streaming_to_openai_account() {
    ...
    assert!(
        ...
        "must emit message_start: {text}"
    );
    assert!(
        text.contains("content_block_delta"),
        "must emit content_block_delta: {text}"
    );
    assert!(
        text.contains("message_stop"),
        "must emit message_stop: {text}"
    );
    // Must NOT contain raw OpenAI chunk format
    assert!(
        !text.contains("chat.completion.chunk"),
        "must not expose raw OpenAI format"
    );
}
...
async fn test_interop_openai_request_to_anthropic_account() {
    ...
    assert!(
        upstream_req.get("system").is_some(),
        "system must be extracted: {upstream_req}"
    );
    // model must be mapped to claude-*
    let sent_model = upstream_req["model"].as_str().unwrap();
    assert!(
        sent_model.starts_with("claude-"),
        "gpt-4o must map to claude-*, got: {sent_model}"
    );
...
async fn test_interop_failover_anthropic_to_openai() {
    ...
    assert_eq!(
        anthro_caps.len(),
        1,
        "Anthropic account must have been tried"
    );
    // OpenAI account was then used
    assert_eq!(
        openai_caps.len(),
        1,
        "OpenAI account must have been used as fallback"
    );
...
async fn test_live_api() {
    ...
    let (token, is_bearer) = if let Ok(t) = std::env::var("CLAUDE_CODE_OAUTH_TOKEN") {
        (t, true)
