...
const TAG_PATTERN = /^(?:core-)?v?\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/;
...
export function validateOptions(options) {
  ...
  if (options.version !== "latest" && !TAG_PATTERN.test(options.version)) {
    throw new Error(`invalid release version "${options.version}"; expected latest, vX.Y.Z, or core-vX.Y.Z`);
  }
...
export function normalizeTag(version) {
  if (version.startsWith("core-v")) return version;
  return `core-${version.startsWith("v") ? version : `v${version}`}`;
}
