...
Coordinator hosts that execute local workers install the separately versioned `@autoswarm/coding-worker` native package. A `local` target is accepted only with a loopback coordinator; Shunt never sends an absolute workspace path to a remote Website3 control plane.

Installing the client workflow does not bypass runtime capability gates. Auto Swarm advertises a target only when that coordinator proves filesystem confinement and exact session-scoped provider/subscription routing. Unconfined local execution and hosted targets without current substrate/gateway proof remain unavailable rather than falling back silently.
